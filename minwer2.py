...
def WER_MinWER(ref, hyp, metric, threshold, save, memory):
    try:
        leftover = save[ref][hyp]
    except KeyError:
        leftover = metric(ref, hyp, memory)
        if ref not in save:
            save[ref] = dict()
        save[ref][hyp] = leftover
    leftover = leftover*0.0001
    return wer(ref, hyp, memory) + leftover
    ...
    if distance <= __MAX__: # to limit the size of graph
        ...
        while minwer < distance:
            for node in level:  # maybe it's better to compute the metric of the first before pursuing?
                corrected_hyp = correcter(ref, hyp, node, base_errors)  # is it a good idea to compute a correcter every time?
                ...
                try:
                    ...
                    if ref not in save:
                        save[ref] = dict()
                    save[ref][corrected_hyp] = score    # SAVE IS NOT UPDATED, NEED TO BE FIXED
                if score < threshold: # lower-is-better
                    return minwer+leftover
            level = get_next_level(level)
            minwer += 1
        return distance+leftover
    else:
        return distance+leftover
        
# minWER:           67.908% with (237, 76, 36) with threshold = 0.024
# minWER+random:    78.797% with (275,  0, 74) with threshold = 0.024
# minWER+leftover:  84.814% with (296,  0, 53) with threshold = 0.024

# WER:              62.462% with (218, 80, 51)
# WER+random:       73.926% with (258,  0, 91)
# WER+leftover:     81.948% with (286,  0, 63)
...
def evaluator(metric, dataset, threshold, memory, picklename_metric, certitude=0.7, verbose=False):
    ignored = 0
    accepted = 0
    correct = 0
    incorrect = 0
    ...
    # recover scores save
    try:
        with open(picklename_metric, "rb") as handle:   # EMPTY - NEED TO BE FIXED
    ...
    for i in range(len(dataset)):
        ...
        if c >= certitude: # if humans are certain about choice
            ...
            scoreA = WER_MinWER(dataset[i]["reference"], dataset[i]["hypA"], metric, threshold, save, memory)
            ref = dataset[i]["reference"]
            hyp = dataset[i]["hypA"]

            scoreB = WER_MinWER(dataset[i]["reference"], dataset[i]["hypB"], metric, threshold, save, memory)
            hyp = dataset[i]["hypB"]
